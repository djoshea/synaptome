...
Source URL:
[http://github.com/djoshea/synaptome](http://github.com/djoshea/synaptome)

Smith Lab @ Stanford University
[http://smithlab.stanford.edu](smithlab.stanford.edu)
12 Jan 2010
...
this reason, you should *add the main synaptome directory to your matlab path*
and then operate out of the local dataset directory.
...
Filtered imaging data resides in ds.trd (as in "training data") instead of ds.sg(:).im. This is grayscale data of the same size (ds.sgdim) and aspect ratio 
...
### Functions
	...
	info is a structure containing metadata returned by some operations.
	 ...
	 filtdat essentially scans the filters directory for a file with the
	 same name as params.type, calls that function internally, and passes
	 along the results, returning an error if the appropriate function is
	 not found. The functions in the filters directory have the same 
	 signature as filtdat, except they can expect to be passed the actual
	 channel data in dat rather than the channel name.
	 
	 See the comments for each invidual filter function for more specific
